...
static void agregar_bloque_busqueda(t_list *lista_agregar,
		t_bloque *bloque_buscado) {
	list_add(lista_agregar, bloque_buscado);

}

t_par_valor_timestamp *crear_valor_timestamp_buscar(unsigned long timestamp,
		char *valor) {
...
static void agregar_valor_timestamp(t_list *timestamp_valor,
		t_par_valor_timestamp *par_valor_timestamp) {
...
void comparar_key_y_agregar_valor(uint16_t key_recv, uint16_t key, char *valor,
		unsigned long timestamp, t_list *timestamp_valor) {
	if (key_recv == key) {
		t_par_valor_timestamp *valor_time = crear_valor_timestamp_buscar(
				timestamp, valor);
...
void buscar_key_bloques(char* bloque_nr, uint16_t key, t_list *timestamp_valor,
		int flag_last_bloque, int size_bloque) {
			...
			if (linea_no_vacia(temp)) {
				...
				if (flag_key_value != 0) {
					if (control == 1 && nr_comas == 2) {
						...
						if (tokens_registro_2[0] != NULL
								&& tokens_registro_2[1] != NULL
								&& tokens_registro_2[2] != NULL)
							comparar_key_y_agregar_valor(
									atol(tokens_registro_2[1]), key,
									strdup(tokens_registro_2[2]),
									atoi(tokens_registro_2[0]),
									timestamp_valor);
						if (tokens_registro[0] != NULL
								&& tokens_registro[1] != NULL
								&& tokens_registro[2] != NULL)
							comparar_key_y_agregar_valor(
									atol(tokens_registro[1]), key,
									strdup(tokens_registro[2]),
									atoi(tokens_registro[0]), timestamp_valor);
					...
					if ((nr_comas == 2)
							&& ((flag_last_bloque == 1)
									&& (bloque_size == size_bloque))) {
						comparar_key_y_agregar_valor(atol(tokens_registro[1]),
								key, strdup(tokens_registro[2]),
								atoi(tokens_registro[0]), timestamp_valor);

					} else if (nr_comas == 2 && bloque_size < size_bloque
							&& (tokens_registro[0] != NULL
									&& tokens_registro[1] != NULL
									&& tokens_registro[2] != NULL)) { //En el medio
						comparar_key_y_agregar_valor(atol(tokens_registro[1]),
								key, strdup(tokens_registro[2]),
								atoi(tokens_registro[0]), timestamp_valor);
					} else {
...
void cargar_timestamp_value(t_list *bloques_buscar, t_list *timestamp_valor,
		uint16_t key) {
	...
	for (int i = 0; i < cant_bloques; i++) {
		...
		buscar_key_bloques(block->name, key, timestamp_valor, flag_last_bloque,
				size_bloque);
	}
	free(block);
}
...
void buscar_bloques_particion(char *tabla, int particion_buscar, int type_flag,
		t_list *bloques_buscar) {
	char* root = string_new();
	string_append(&root, puntoDeMontaje);
...
t_list *filtrar_registros_particion(t_list *particion_encontrada, uint16_t key) {
	...
	registros_encontrados =
			list_filter(
					((t_particion *) list_get(particion_encontrada, 0))->lista_registros,
					coincide_key);
	return registros_encontrados;

}

t_par_valor_timestamp *filtrar_timestamp_mayor(t_list *timestamp_valor,
		int list_size) {
	int i;
...
//		enviarIntConHeader(socket_cliente, ERROR, RESPUESTA);//no testeado
		...
		t_par_valor_timestamp *timestamp_value_max = filtrar_timestamp_mayor(
				timestamp_valor, list_size(timestamp_valor));
//		printf("El valor de la key ingresada es: %s\n", timestamp_value_max->valor);
		...
		if (timestamp_value_max->valor != NULL) {
			...
			registroEncontrado->value = string_duplicate(
					timestamp_value_max->valor);
