...
//	pthread_create(&hilo_consola, NULL, (void *)iniciar_consola, NULL);
	...
	socket_server = iniciar_servidor();
	if (socket_server == -1) {
		perror("no se pudo iniciar el server correctamente");
		log_destroy(logger);
		close(socket_server);
		return EXIT_FAILURE;
	}
...
//cuando se ingresa una linea vacia la consola se detiene
void iniciar_consola() {
	char *comando_ingresado = readline("<");

	while(comando_ingresado[0] != '\0') {
		add_history(comando_ingresado);
		procesar_comando(comando_ingresado);
		comando_ingresado = readline("<");
	}
}

//procesa un comando VALIDO (sin espacios en los costados ni con cosas raras que no se puedan procesar)
void procesar_comando(char* comando) {
	LFS_OPERACION operacion = get_operacion(comando);

	switch (operacion) {
		case SELECT:
			rutina_select(comando); //comando es el leído por el readline
			break;

		case INSERT:
			rutina_insert(comando);
			break;

		case CREATE:
			rutina_create(comando);
			break;
...
//retorna una constante que representa la operacion solicitada: select, insert...
LFS_OPERACION get_operacion(char* comando) {
	if (string_starts_with(comando, "SELECT")) {
		return SELECT;
	} else if (string_starts_with(comando, "INSERT")) {
		return INSERT;
	} else if (string_starts_with(comando, "DROP")) {
		return DROP;
	} else if (string_starts_with(comando, "CREATE")) {
		return CREATE;
	} else if (string_starts_with(comando, "DESCRIBE")) {
		return DESCRIBE;
	} else {
...
int obtener_particiones_metadata(char* tabla)
{
	char root[64] = "../Tablas/";
	strcat(root, tabla);
	strcat(root, "/metadata.config");

	t_config* config = config_create(root);
...
int contar_comas(char *temp)
{
 ...
 {
	 char root[64] = "";
	 sprintf(root, "../Bloques/bloque%s.bin", bloque_nr);
	 int bloque_size = 0;
	 FILE *f;
	 f = fopen(root, "rb");
	 if(f==NULL){
...
//				 	printf("MOD: %s\n", array_aux);
 ...
 int size_of_bloque(t_bloque *block)
 {
	 ...
	 char root[128] = "";
	 FILE *bf;
	 sprintf(root, "../Bloques/bloque%s.bin", bloque->name);
	 bf = fopen(root, "r+b");
...
int existe_tabla(char *tabla)
{
	...
	//Verifico existencia de tabla
	FILE *f;
	f = fopen(root, "r");
	if(f==NULL){
		printf("La tabla solicitada no existe\n");
		return 0;
	}else{
		fclose(f);
		return 1;
...
void rutina_insert(char* comando)
{
	...
	if(strlen(value)>LFS_config.tamanio_value){
...
void guardar_bitarray(t_bitarray *bitarray, long int index)
{
	FILE *fp;
	char *root;
	int size = strlen("../Metadata/Bitmap.bin");
	root = malloc(size);
...
void crear_particiones(char *dir, int particiones)
{
	...
	sprintf(size_aux, "%d", FS->block_size);
	...
	for(i = 0; i < particiones; i++){
		...
		FILE *fp;
		fp = fopen(root, "w+b");
		int free_block = elegir_bloque_libre(FS->nr_blocks);
		if(free_block == -1){
	...
	if(flag_close_file == 1 && fp_dump != NULL){
		...
		if(space_full == 0){ //Escribo registro
			...
			if((j < length_registro) && ((suma_size == espacio_libre) || (suma_size > espacio_libre))){ //Si se queda sin espacio
				fclose(fp_dump);
				bloque_dump = elegir_bloque_libre(FS->nr_blocks);
...
void cargar_configuracion_inicial() {
	...
	LFS_config.retardo = config_get_int_value(config, "RETARDO");
	LFS_config.tamanio_value = config_get_int_value(config, "TAMAÑO_VALUE");
...
{
	t_config* config = config_create("../Metadata/Metadata.bin");
	FS->block_size = config_get_int_value(config, "BLOCK_SIZE");
	FS->nr_blocks = config_get_int_value(config, "BLOCKS");
	FS->magic_number = malloc(strlen(config_get_string_value(config, "MAGIC_NUMBER")) * sizeof(char));
	strcpy(FS->magic_number, config_get_string_value(config, "MAGIC_NUMBER"));

	config_destroy(config);

	crear_bitarray(FS->nr_blocks);
}


//deja el server escuchando y retorna el socket servidor o -1 en caso de fallo
int iniciar_servidor() {
	struct sockaddr_in direccion_servidor;
	direccion_servidor.sin_family = AF_INET;
	direccion_servidor.sin_addr.s_addr = INADDR_ANY;
	direccion_servidor.sin_port = htons(LFS_config.puerto_escucha);

	int socket_servidor = socket(AF_INET, SOCK_STREAM, 0);

	/*** para no tener que esperar 2 minutos para que vuelva a funcionar el bind ***/
	int activado = 1;
	setsockopt(socket_servidor, SOL_SOCKET, SO_REUSEADDR, &activado, sizeof(activado));

	if (bind(socket_servidor, (void*)&direccion_servidor, sizeof(direccion_servidor)) == -1) {
		return -1;
	}
...
void atender_cliente(int socket_escucha) {
	...
	int nuevo_cliente = accept(socket_escucha, (void*) &direccion_cliente, &tamanio_direccion);

	printf("se conecto un cliente\n");
	send(nuevo_cliente, "holi\n", 7, 0);
